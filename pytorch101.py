...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    #y = x[::-1, :]
    M = x.shape[0]
    id = torch.arange(M, device=x.device) 
    r = (M - 1) - id
    y = x[r, :]
    ##########################################################################
    #                            END OF YOUR CODE                            #
