...
def svm_loss_vectorized(
    ...
    num_train = X.shape[0]
    scores = X.matmul(W)
    c_scores = scores[torch.arange(num_train), y].unsqueeze(1) #获取每个样本的正确类别得分
    margins = scores - c_scores + 1 
    margins[torch.arange(num_train), y] = 0              #将正确类别的 margin 设置为 0
    margins[margins < 0] = 0
    ...
    # loss.                                                                     #
    #############################################################################
    coeffs = (margins > 0).double()    # margin > 0 的地方为 1，否则为 0
    counts = torch.sum(coeffs, dim=1)    #计算每个样本有多少个 margin > 0 
    coeffs[torch.arange(num_train), y] -= counts
    dW = X.t().matmul(coeffs)/num_train
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def softmax_loss_vectorized(
    ...
    scores = X.matmul(W)
    stable_scores = scores - torch.max(scores, dim=1, keepdim=True)[0] 
    ...
    dW = X.t().matmul(dScores) / num_train
    dW += 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
