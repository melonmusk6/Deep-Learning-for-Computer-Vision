...
def nn_forward_backward(
    ...
    data_loss = torch.mean(-torch.log(probs[range(N),y]))
    ...
    dh1 = probs @ W2.t() 
    dh1_pre_relu = dh1 * (h1 > 0).type(dh1.dtype) 

    grads['W1'] = X.t() @ dh1_pre_relu 
    grads['b1'] = torch.sum(dh1_pre_relu, dim=0) 
    grads['W1'] += 2 * reg * W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_get_search_params():
    ...
    regularization_strengths =torch.logspace(start = -4,end = -1,steps = 5)
    hidden_sizes = [128, 256, 512]
    learning_rate_decays = [1.0, 0.99, 0.9]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
        ...
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    cnt += 1
                    print(f"Training combination {cnt}/{total_combinations}: lr={lr}, hs={hs}, reg={reg}, decay={lr_decay}")
                    net = TwoLayerNet(input_size, hs, output_size, dtype=dtype, device=device)
    ...
    print(f"Finished hyperparameter search. Best validation accuracy: {best_val_acc:.4f}")
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
