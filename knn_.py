...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dis = x_train_re[i] - x_test_re[j]
            dists[i, j] = torch.sum(dis ** 2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        dis = x_train_re[i] - x_test_re
        dists[i, :] = torch.sum(dis ** 2, dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_sum = torch.sum(x_train_re**2, dim=1, keepdim=True) 
    test_sum = torch.sum(x_test_re**2, dim=1) 
    dists = train_sum + test_sum - 2 * torch.mm(x_train_re, x_test_re.t())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        dis, id = torch.topk(dists[:, j], k, largest=False) 
        val = y_train[id] 
        y_pred[j] = torch.mode(val).values
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        dis = compute_distances_no_loops(self.x_train, x_test)
        y_test_pred = predict_labels(dis, self.y_train, k=k)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    # A dictionary holding the accuracies for diserent values of k that we
    ...
    # diserent accuracies we found trying `KnnClassifier`s using k neighbors.
    k_to_accuracies = {}
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            res = knn.check_accuracy(x_train_folds[i], y_train_folds[i], k=k, quiet=True)

            k_to_accuracies[k].append(res)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in sorted(k_to_accuracies.keys()):
        now = torch.tensor(k_to_accuracies[k]).mean().item()
        if now > maxx:
            maxx = now
            best_k = k
    ##########################################################################
